...
async fn js_repl_link_failures_keep_prior_module_state() -> Result<()> {
    ...
    assert_js_repl_err(
        ...
        "call-2",
        "Unsupported import specifier \"./foo\"",
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn js_repl_failed_cells_do_not_commit_short_circuited_logical_var_assignments() -> Result<()>
{
    skip_if_no_network!(Ok(()));
    ...
    assert_js_repl_err(&mocks[0].single_request(), "call-1", "boom");
    assert_js_repl_ok(&mocks[1].single_request(), "call-2", "ReferenceError 2 3");
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn js_repl_failed_cells_preserve_pre_declaration_var_writes() -> Result<()> {
    skip_if_no_network!(Ok(()));

    let server = responses::start_mock_server().await;
    let mocks = run_js_repl_sequence(
        &server,
        "run js_repl through pre-declaration var writes",
        &[
            (
                "call-1",
                "x = 5; y = 1; y += 2; z = 1; z++; throw new Error(\"boom\"); var x, y, z;",
            ),
            ("call-2", "console.log(x, y, z);"),
        ],
    )
    .await?;

    assert_js_repl_err(&mocks[0].single_request(), "call-1", "boom");
    assert_js_repl_ok(&mocks[1].single_request(), "call-2", "5 3 2");

    Ok(())
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn js_repl_failed_cells_do_not_commit_shadowed_nested_var_writes() -> Result<()> {
    skip_if_no_network!(Ok(()));

    let server = responses::start_mock_server().await;
    let mocks = run_js_repl_sequence(
        &server,
        "run js_repl through shadowed nested var writes",
        &[
            (
                "call-1",
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn js_repl_failed_cells_do_not_recover_nested_var_writes_inside_assignments() -> Result<()> {
    skip_if_no_network!(Ok(()));

    let server = responses::start_mock_server().await;
    let mocks = run_js_repl_sequence(
        &server,
        "run js_repl through unsupported nested var writes",
        &[
            (
                "call-1",
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn js_repl_failed_cells_do_not_recover_partial_var_destructuring() -> Result<()> {
    skip_if_no_network!(Ok(()));
